...
class Quarantine {
  ...
  public isThresholdReached(
    extraErrors?: TestRunErrorFormattableAdapter[]
  ): boolean {
  ...
  public isFirstAttemptSuccessful(
    extraErrors: TestRunErrorFormattableAdapter[]
  ): boolean {
    const { failedTimes, passedTimes } = this._getAttemptsResult(extraErrors);
  ...
  private _getAttemptsResult(
    ...
    if (extraErrors) {
      if (extraErrors.length) failedTimes += extraErrors.length;
      else passedTimes += 1;
    }
...
export default class TestRunController extends AsyncEventEmitter {
  ...
  private readonly _testRunCtor:
    | LegacyTestRun['constructor']
    | TestRun['constructor'];
  ...
  private static _getTestRunCtor(
    test: Test,
    opts: Dictionary<OptionValue>
  ): LegacyTestRun | TestRun {
    if (opts.TestRunCtor) return opts.TestRunCtor;

    return (test as LegacyTestRun).isLegacy ? LegacyTestRun : TestRun;
  }

  private async _createTestRun(
    connection: BrowserConnection
  ): Promise<TestRun | LegacyTestRun> {
    const screenshotCapturer = this._screenshots.createCapturerFor(
      this.test,
      this.index,
      this._quarantine,
      connection,
  ...
  private async _endQuarantine(): Promise<void> {
    if ((this._quarantine as Quarantine).attempts.length > 1)
      this.testRun.unstable =
        (this._quarantine as Quarantine).getPassedAttempts().length > 0;
  ...
  private _shouldKeepInQuarantine(): boolean {
    ...
    const shouldKeepRunning = isFirstAttempt
      ? hasErrors
      : !(this._quarantine as Quarantine).isThresholdReached();

    if (shouldKeepRunning) {
      console.log(
        `--------------------------Yoni----------_shouldKeepInQuarantine--shouldKeepRunning : ${shouldKeepRunning}----${this.test.name}--------`
      );
      console.log(
        `--------------------------Yoni----------_shouldKeepInQuarantine--isThresholdReached : ${(this
          ._quarantine as Quarantine).isThresholdReached()}----${
          this.test.name
        }--------`
      );

      console.log(
        `--------------------------Yoni----------_shouldKeepInQuarantine--hasErrors : ${hasErrors}----${this.test.name}--------`
      );
      ...
      const errorsNames = this.testRun.errs;
      console.log(
        `--------------------------Yoni----------_shouldKeepInQuarantine--hasErrors : ${hasErrors}----${this.test.name}--------`
      );
    }
  ...
  private async _testRunDoneInQuarantineMode(): Promise<void> {
    if (this._shouldKeepInQuarantine()) await this._keepInQuarantine();
    else await this._endQuarantine();
  }

  private async _testRunDone(): Promise<void> {
    if (this._quarantine) await this._testRunDoneInQuarantineMode();
    else await this._emitTestRunDone();
  }
  ...
  private async _emitTestRunDone(): Promise<void> {
    ...
    await this._fixtureHookController.runFixtureAfterHookIfNecessary(
      this.testRun
    );
  ...
  private async _testRunBeforeDone(): Promise<void> {
    ...
    if (!raiseEvent) {
      const isSuccessfulQuarantineFirstAttempt =
        this._isFirstQuarantineAttempt() && !this.testRun.errs.length;
      const isAttemptsThresholdReached = (this
        ._quarantine as Quarantine).isThresholdReached(this.testRun.errs);

      raiseEvent =
        isSuccessfulQuarantineFirstAttempt || isAttemptsThresholdReached;
    }

    if (raiseEvent) await this.emit('test-run-before-done');
  }

  private _testRunDisconnected(connection: BrowserConnection): Promise<void> {
    this._disconnectionCount++;

    const disconnectionThresholdExceedeed =
  ...
  private _assignTestRunEvents(
    testRun: TestRun | LegacyTestRun,
    connection: BrowserConnection
  ): void {
    testRun.on('action-start', async (args: ActionEventArg) =>
      this._emitActionStart(Object.assign(args, { testRun }))
    );
    testRun.on('action-done', async (args: ActionEventArg) =>
      this._emitActionDone(Object.assign(args, { testRun }))
    );
  ...
  public async start(connection: BrowserConnection): Promise<string | null> {
    ...
    const hookOk = await this._fixtureHookController.runFixtureBeforeHookIfNecessary(
      testRun
    );
